		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Execute Installation"), function() {
				...
				const d = new frappe.ui.Dialog({
					title: __("Live Shell Output - Node Installation"),
					size: "large",
				...
				const $log = $wrapper.find("#live-node-log");
				...
				frm.call({
					...
					callback(r) {
						frappe.realtime.off(event_name);
						$log.append("\n[SUCCESS] Node installation & verification complete.\n");
						$log.scrollTop($log[0].scrollHeight);
						frappe.msgprint({
							title: __("Installation Complete"),
							indicator: "green",
							message: __("Successfully installed Node version.")
						});
						frm.reload_doc();
					}
				});
