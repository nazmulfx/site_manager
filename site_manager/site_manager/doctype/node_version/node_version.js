frappe.ui.form.on("Node Version", {
	setup(frm) {
		frappe.realtime.on("node_version_shell_log", (data) => {
			if (data && data.line) {
				if ($("#live-node-log").length) {
					$("#live-node-log").append(frappe.utils.escape_html(data.line));
					$("#live-node-log").scrollTop($("#live-node-log")[0].scrollHeight);
				}
			}
		});
	},
	before_save(frm) {
		if (frm.is_new()) {
			const d = new frappe.ui.Dialog({
				title: __("Live Shell Log - Node Installation"),
				size: "large",
				fields: [{ fieldtype: "HTML", fieldname: "log_html" }],
				primary_action_label: __("Close"),
				primary_action() {
					frappe.realtime.off("node_version_shell_log");
					d.hide();
				}
			});
			...
			$wrapper.html(`
				<div style="background: #121212; border-radius: 8px; border: 1px solid #2a2a2a; overflow: hidden;">
					<div style="background: #1e1e1e; padding: 8px 15px; border-bottom: 1px solid #2a2a2a; display: flex; align-items: center; justify-content: space-between;">
						<span style="color: #aaa; font-size: 12px; font-family: monospace;">bash &bull; install_node_version.sh</span>
						<span style="display: inline-block; width: 10px; height: 10px; background: #00ff66; border-radius: 50%; box-shadow: 0 0 8px #00ff66;"></span>
					</div>
					<div id="live-node-log" style="
						color: #4af626;
						...
						line-height: 1.5;
					">$ Executing install_node_version.sh...\n\n</div>
				</div>
			`);
			frm._live_dialog = d;
		}
